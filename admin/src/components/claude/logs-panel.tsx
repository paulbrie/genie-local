...
/**
 * Raw viewer for any log-like file under ~/.claude — session JSONL transcripts,
 * debug logs, shell snapshots. Modeled on the main Logs page's viewer.
 */
export function LogsPanel() {
  ...
  const loadList = useCallback(async () => {
    try {
      const res = await fetch(`${BASE_PATH}/api/claude/logs`, {
        cache: "no-store",
      });
  ...
  const loadTail = useCallback(async (file: string, byteWindow: string) => {
    try {
      const res = await fetch(
        `${BASE_PATH}/api/claude/logs/tail?file=${encodeURIComponent(
          file,
        )}&bytes=${byteWindow}`,
        { cache: "no-store" },
      );
                ...
                <li key={f.path}>
                  ...
                  >
                    ...
                    <span className="text-[11px] text-muted-foreground">
                      {formatBytes(f.size)} ·{" "}
                      {formatRelativeTime(new Date(f.mtimeMs))}
                    </span>
                  </button>
                </li>
              ))}
      ...
      <section className="flex min-w-0 flex-1 flex-col gap-2">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            <Label htmlFor="claude-log-bytes" className="text-xs text-muted-foreground">
              Tail
            </Label>
            <Select value={bytes} onValueChange={(v) => v && setBytes(v)}>
              <SelectTrigger id="claude-log-bytes" className="h-8 w-[110px]">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                {BYTE_OPTIONS.map((o) => (
